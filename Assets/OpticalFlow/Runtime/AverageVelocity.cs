...
namespace Bonjour.Vision
{
    public class AverageVelocity : MonoBehaviour
    {
        ...
        [Tooltip("Movement speed of tracker")] public float MoveSpeed = 2f;
        [Tooltip("Movement speed of tracker back to 0")] public float ReturnSpeed = 0.05f;
        ...
        private void ComputeAverageVelocity(){
            ...
            //X
            if (Tracker.transform.localPosition.x > 5 || Tracker.transform.localPosition.x < -5)
            {
                myVector.x = 0;
            }
            else
            {
                myVector.x = averageVelocity.x;
            }

            //Y

            if (Tracker.transform.localPosition.y > 5 || Tracker.transform.localPosition.y < -5)
            {
                myVector.y = 0;
            }
            else
            {
                myVector.y = averageVelocity.y;
            }

            Tracker.transform.localPosition = Vector3.Lerp(Tracker.transform.localPosition, Tracker.transform.localPosition + myVector, MoveSpeed);

            Tracker.transform.localPosition = Vector3.Lerp(Tracker.transform.localPosition, new Vector3(0, 0, 0), ReturnSpeed);
        }
